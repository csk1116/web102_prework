    ...
    for (let game of games) {
        ...
        gameCard.innerHTML = `
            ...
            <p style="margin-top: auto;">
                Goal: $${game.goal.toLocaleString('en-US')} Raised: $${game.pledged.toLocaleString('en-US')}
            </p>
        `;
...
const unfundedCount = GAMES_JSON.reduce((count, game) => {
    return count + (game.goal > game.pledged ? 1 : 0);
}, 0);

// create a string that explains the number of unfunded games using the ternary operator
const displayStr = `<p style="font-weight:bold;">A total of $${totalPledged.toLocaleString('en-US')} has been raised for
                    ${GAMES_JSON.length} games. Currently, ${unfundedCount} ${unfundedCount > 1 ? "games remain " : "game remains "}
                    unfunded. We need your help to fund these amazing games!</p>`;

// create a new DOM element containing the template string and append it to the description container
const unfundedInfo = document.createElement("p");
unfundedInfo.innerHTML = displayStr;
